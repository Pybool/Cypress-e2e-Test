...
function selectSeatsByCompartments(initialIndex, compartments) {
  cy.get('button.chakra-tabs__tab')
    .contains(compartments[initialIndex].name, { timeout: x6 })
    .as('selected')
    .scrollIntoView()
    .should('exist')
    .and('be.visible')
  ...
  cy.get('@selected').should('have.attr', 'aria-selected', 'true')
  cy.get('div.chakra-modal__body')
    .find('div.chakra-tabs__tablist')
    .eq(compartments[initialIndex].index + 1)
    .as('comps')
    .children()
    .then((comp) => {
  ...
  cy.get('@comps')
    ...
    .then(($siblings) => {
      ...
      if ($buttons.length > 0) {
        ...
        cy.wrap(Cypress.$($buttons[0])).should(
          'have.css',
          'background-color',
          'rgb(15, 28, 42)',
        )
    ...
    cy.get('div.chakra-modal__body')
      ...
      .then((compartments) => {
        ...
        for (let i = 0; i < pills.length; i++) {
          if (
            step == '2 adults 2 children' &&
            pills[i].innerText.includes('4-')
          ) {
            matchPill.push({ name: pills[i].innerText, index: i })
          }
          if (
            step.includes('3 adults 3 children') &&
            pills[i].innerText.includes('6-')
          ) {
            matchPill.push({ name: pills[i].innerText, index: i })
          }
        }
...
Then(
  'For {string} I select the needed compartments and choose appropriate seats for each respectively',
  (step) => {
    const keyVals = {
      '1 adults 0 children': singleAdult,
      '3 adults 3 children': fourChildrenThreeAdults,
      '2 adults 2 children': twoChildrentwoAdults,
      '3 adults 3 children in closed carriages':
        fourChildrenThreeAdultsClosedCarriage,
      '3 adults 3 children in semi-open carriages':
        fourChildrenThreeAdultsSemiOpenCarriage,
      '3 adults 3 children in open carriages':
        fourChildrenThreeAdultsOpenCarriage,
      '3 adults 3 children in Accessible carriages':
        fourChildrenThreeAdultsAccessibleCarriage,
    }
    for (let i = 0; i < 1; i++) {
      cy.get('button.chakra-button')
        .contains('Next Day', { timeout: x6 })
        .click({ force: true })
    }
    cy.get('h3.chakra-heading')
      .contains(Cypress.env('product'), { timeout: x6 })
