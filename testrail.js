  ...
  if (test.length > 0) {
    ...
    if (section == undefined) {
      var response = await axios.post(
        ...
        { suite_id: suiteId, name: testEnvironment },
        opt
      );
      await getsectionsList();
      var response = await axios.get(
        `${options.url}/index.php?/api/v2/get_cases/${projectId}&suite_id=${suiteId}&section_id=${section.id}`,
        opt
      );
    } else {
      var response = await axios.get(
      ...
      if (cases.length > 0) {
        ...
        try {
          let resp = await axios.post(
            `${options.url}/index.php?/api/v2/add_case/${section.id}`,
            genratedCase[i][0],
            opt
          );
...
const exportTest = async (createDirs) => {
  ...
  if (fs.existsSync(resultDir)) {
    ...
    console.log(
      `[TEST EXPORTER :] ${new Date().toLocaleDateString()}: Exported ${
        allFiles.length - 1
      } files`
    );
  } else {
